...
if windows:
    print("I've determined you are on a Windows machine. If this is not the "
          "case,\n\tsomething is wrong and you'll have to download "
          "the scripts manually.\n")
    def_path = f"{os.getenv('APPDATA')}\\mpv\\scripts\\"
    sep = "\\"
else:
    print("I've determined you're on a UNIX-like OS. "
          "Thanks for making a somewhat sane decision.")
...
filelist = json.loads(
    jsonreq.read().decode(
        jsonreq.headers.get_content_charset()
    ).replace("\r\n", "\n")
)
...
def install_script(data: Dict, name: str):
    ...
    if len(data["optional"]) > 0:
        joiner = "\n - "
        opts = input(f"There are optional script files:{joiner}"
                     f"{joiner.join(data['optional'])}\nThese are recommended "
                     f"to have, would you like to install these? "
                     f"[Y/n]\n\n> ") or "y"
    targets = (data["required"] + data["optional"]
               if opts.lower().startswith("y")
               else data["required"])
    out_path = (input(f"Please provide the full path for your mpv scripts "
    ...
    for t in targets:
        ...
        content = scriptreq.read().decode(
            scriptreq.headers.get_content_charset()
        ).replace("\r", "").split("\n")
...
while not select_script(filelist):
    pass

# If we get here without errors, we're done. And it's good.
print("\nShould've been successful installing all requested scripts. "
      "Thanks for using me and don't forget to run me again for updates!")
# Bedtime
exit(0)
